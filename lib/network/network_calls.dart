...
class RestDatasource {
  ...
  static final Update_DELIVERY_Customer = BASE_URL + "/delivery-lines/addresses/update/";
  static final Update_User_Location = BASE_URL + "/driver/location/update";
  Map userHeader = <String, String>{"Content-type": "application/json", "Accept": "application/json","Authorization":"\$2y\$12\$JVeIGlnT/q/1Orgg8DDYJOn70PkFFaoPcJlPTlK3P5UAlD0wFwLQS","Device-id":"1f493a6a-0d86-49b5-ad1b-ff775bd11582","os":"ios","language":"ar"};


  Future<User> login(String username, String password)  {
    return http.post(LOGIN_URL, body: convert.jsonEncode({
      "email": username,
      "password": password
    }),headers: userHeader).then((http.Response response) {
      final int statusCode = response.statusCode;
  ...
  Future<DeliveryLines> getDeliveryLines()  {
    ...
    return http.get(LINES_URL,headers: userHeader).then((http.Response response) {
      final int statusCode = response.statusCode;
  ...
  Future<Response> updateDeliveryItem(SheetItem sheetItem)  {
    userHeader["Token"] = sharedPrefs.token;
    var body = convert.jsonEncode({"location": {
      "latitude": sheetItem.address.location.latitude,
      "longitude": sheetItem.address.location.longitude
    }});
    return http.post(DELIVERY_Item+sheetItem.id, body:body,headers: userHeader).then((http.Response response) {
      final int statusCode = response.statusCode;
  ...
  Future<SheetsView> getDeliveryView(String id)  {
    userHeader["Token"] = sharedPrefs.token;
    return http.get(DELIVERY_VIEW+id+"?sheet_id=",headers: userHeader).then((http.Response response) {
      final int statusCode = response.statusCode;
  ...
  Future<CustomerView> getCustomerView(String id)  {
    userHeader["Token"] = sharedPrefs.token;
    return http.get(DELIVERY_Customer+id,headers: userHeader).then((http.Response response) {
      final int statusCode = response.statusCode;
  ...
  Future<CustomerView> updateCustomerView(String id,String addressId,double long,double lat)  {
    final random = Random();
   ...
   // lat = 31.906436;
    ...
    return http.post(Update_DELIVERY_Customer+id+"/"+addressId, body:body,headers: userHeader).then((http.Response response) {
      final int statusCode = response.statusCode;
  ...
  Future<bool> updateUserLocation(double long,double lat)  {
    ...
    return http.post(Update_User_Location, body:body,headers: userHeader).then((http.Response response) {
      final int statusCode = response.statusCode;
